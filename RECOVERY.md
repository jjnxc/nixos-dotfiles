...
6. Generate a fresh `hardware-configuration.nix` for the new hardware, but
   **manually re-add your `/home` fileSystems block** (UUID may differ if it's a
   new physical drive — check with `blkid`):
```bash
   nixos-generate-config --root /mnt
```
   Then edit the generated `hardware-configuration.nix` — or just copy the
   `fileSystems."/home"` block from your repo's version, fixing the UUID if needed.
